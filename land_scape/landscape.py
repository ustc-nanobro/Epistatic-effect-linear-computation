...
class Protein_Landscape:
    ...
    def __init__(
        ...
        if not data and not csv_path:
            raise Exception.FileNotFoundError(
                "Data must be provided as numpy array or csv file"
            )

        if csv_path and data:
            print(
                """Both a filepath and a data array has been provided. The CSV is
    ...
    def lengthen_sequences(self, seq_len, AAs=None, mut_indices=False):
        ...
        if mut_indices.__class__.__name__ == "ndarray":
            assert (
                len(mut_indices) == sequences.shape[1]
            ), "The index array must have the same length as the original sequence"
            idxs = mut_indices
    ...
    def return_lengthened_data(
        self, seq_len, AAs=None, mut_indices=False, split=0.8, shuffle=True
    ):
        """
    ...
    def indexing(self, distances=None, percentage=None, positions=None):
        ...
        if percentage is not None:
            ...
            for idx in np.random.choice(
                np.arange(len(self)), size=int(len(self) * percentage), replace=False
            ):
                idxs[idx] = 1
        ...
        if positions is not None:
            ...
            for pos in not_positions:
                ...
                working = np.logical_and(
                    temp, np.logical_not(self.sequence_mutation_locations[:, pos])
                )
            return working
    ...
    def rs_ruggedness(self, log_transform=False, distance=None, split=1.0):
        ...
        lin_model = LinearRegression(n_jobs=8).fit(x_train, y_train)
...
if __name__ == "__main__":
    # aa = Protein_Landscape(csv_path="data.csv").get_rs_ex()
    # print(aa)
    ...
    for i in range(11):
        ...
        test2 = Protein_Landscape(
            csv_path=f"/Users/mac/code/NK_Benchmarking/rhla13_exp/K{i}/V1.csv"
        ).get_rs_ex()
        test3 = Protein_Landscape(
            csv_path=f"/Users/mac/code/NK_Benchmarking/rhla13_exp/K{i}/V2.csv"
        ).get_rs_ex()
        test4 = Protein_Landscape(
            csv_path=f"/Users/mac/code/NK_Benchmarking/rhla13_exp/K{i}/V3.csv"
        ).get_rs_ex()
        test5 = Protein_Landscape(
            csv_path=f"/Users/mac/code/NK_Benchmarking/rhla13_exp/K{i}/V4.csv"
        ).get_rs_ex()
        test6 = Protein_Landscape(
            csv_path=f"/Users/mac/code/NK_Benchmarking/rhla13_exp/K{i}/V5.csv"
        ).get_rs_ex()
        test7 = Protein_Landscape(
            csv_path=f"/Users/mac/code/NK_Benchmarking/rhla13_exp/K{i}/V6.csv"
        ).get_rs_ex()
        test8 = Protein_Landscape(
            csv_path=f"/Users/mac/code/NK_Benchmarking/rhla13_exp/K{i}/V7.csv"
        ).get_rs_ex()
        test9 = Protein_Landscape(
            csv_path=f"/Users/mac/code/NK_Benchmarking/rhla13_exp/K{i}/V8.csv"
        ).get_rs_ex()
        test10 = Protein_Landscape(
            csv_path=f"/Users/mac/code/NK_Benchmarking/rhla13_exp/K{i}/V9.csv"
        ).get_rs_ex()
        ...
        # 将DataFrame写入XLSX文件
        output_file = f"/Users/mac/code/NK_Benchmarking/rhla1_3_exp_result/k{i}.xlsx"
        df.to_excel(output_file, index=False)
        print(f"数据已写入 {output_file}")
        e_rs_std_deviation = np.std(extrema_ruggedness_elements)
        e_rs_mean = sum(extrema_ruggedness_elements) / len(extrema_ruggedness_elements)

        rs_std_deviation = np.std(RS_ruggedness_elements)
        rs_mean = sum(RS_ruggedness_elements) / len(RS_ruggedness_elements)
        rs_median_nk = np.median(RS_ruggedness_elements)
        max_value = max(RS_ruggedness_elements)
        min_value = min(RS_ruggedness_elements)

        # 从列表中移除最大值和最小值
        RS_ruggedness_elements.remove(max_value)
        RS_ruggedness_elements.remove(min_value)

        # 计算剩余值的平均数
        rs_average_without_extremes = sum(RS_ruggedness_elements) / len(
            RS_ruggedness_elements
        )
        # 将中位数和均值保存到txt文件
        output_file = f"/Users/mac/code/NK_Benchmarking/rhla1_3_exp_result/result.txt"
        with open(output_file, "a") as file:
            file.write(f"k={i}标准差: {rs_std_deviation}\n")
            file.write(f"k={i}中位数: {rs_median_nk}\n")
            file.write(f"k={i}平均值: {rs_mean}\n")
            file.write(f"k={i}去出最大值最小值平均数: {rs_average_without_extremes}\n")
            file.write(f"k={i}ex_rs平均值: {e_rs_mean}\n")

        print(f"中位数和均值已保存到 {output_file}")
        '''